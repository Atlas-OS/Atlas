...
#[path = "complex_script_breaks.rs"]
mod complex_script_breaks;
...
impl LineLayout {
    ...
    fn compute_wrap_boundaries(
        ...
        let complex_breaks = complex_script_breaks::for_text(text);
        let mut last_cluster = None;
        let mut previous_glyph_index = None;
        ...
        while let Some((boundary, ch, x)) = glyphs.next() {
            ...
            let first_glyph_at_index = previous_glyph_index != Some(byte_index);
            previous_glyph_index = Some(byte_index);
            if let Some(breaks) = &complex_breaks {
                if first_glyph_at_index && breaks.clusters.binary_search(&byte_index).is_ok() {
                    last_cluster = Some((boundary, x));
                }
                if first_glyph_at_index
                    && first_non_whitespace_ix.is_some()
                    && breaks.opportunities.binary_search(&byte_index).is_ok()
                {
            ...
            if width > wrap_width && boundary > last_boundary {
                ...
                } else if let Some((cluster, cluster_x)) = last_cluster {
                    // An over-wide word may break only between grapheme clusters.
                    if cluster <= last_boundary {
                        prev_ch = ch;
                        continue;
                    }
                    last_boundary = cluster;
                    last_boundary_x = cluster_x;
                } else {
