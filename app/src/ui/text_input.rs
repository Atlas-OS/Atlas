// Adapted from Zed GPUI examples/input.rs (Apache-2.0).
...
use gpui::{
    ...
    MouseDownEvent, MouseMoveEvent, MouseUpEvent, PaintQuad, Pixels, Point, ShapedLine, SharedString, Style,
    TextRun, UTF16Selection, UnderlineStyle, Window, actions, div, fill, point, prelude::*, px, relative,
    size,
};
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        let (display_text, text_color) = if content.is_empty() {
            (input.placeholder.clone(), cx.theme().text_secondary)
        } else {
            (content, style.color)
        };
...
impl Render for TextInput {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .rounded(px(4.))
            .overflow_hidden()
            .line_height(px(22. * cx.theme().text_scale))
            .text_size(px(14. * cx.theme().text_scale))
            .text_color(cx.theme().text_primary)
            .child(
                div()
                    .h(px(22. * cx.theme().text_scale + 12.))
                    .w_full()
                    .px(px(10.))
                    .py(px(6.))
