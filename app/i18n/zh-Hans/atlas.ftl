...
detail-reboot-file-renames = Windows 正在等待下次重启时替换或删除文件（{ $files }）。Xbox Gaming Services 等部分应用每次重启后都会留下此标记。Atlas 仍可继续安装。
...
security-banner-absent-message = 之前的 Atlas 安装已将其移除，因此这里没有需要关闭的项目。准备好后即可继续。
...
prepare-failed = 部分更新未能完成。请查看下方错误，解决问题后重试。
...
prepare-resumed = Windows 已重新启动。你的 Atlas 选项已恢复。请在安装 Atlas 前继续更新。
prepare-continue = 继续更新
prepare-saving-restart = 正在保存你的选项，并设置在 Windows 重新启动后打开 Atlas…
prepare-restart-save-failed = 无法保存你的选项。请在重新启动前重试。
prepare-restart-registration-failed = 你的选项已保存，但无法设置自动重新打开。请重试，或重新启动 Windows 后手动打开 Atlas。
prepare-restart-failed = Windows 无法重新启动。请重试，或通过 Windows 重新启动。你的选项已保存，Atlas 已设置为重新打开。
...
prepare-progress-unchanged = 已有 { $minutes } 分钟未报告进度变化。大型更新可能需要较长时间。请打开诊断查看详情。
prepare-report-delayed = 已有 { $seconds } 秒未收到状态信息。请打开诊断检查更新进程。

prepare-affected-app = 受影响的应用
prepare-app-in-use = 关闭 { $app } 及其窗口，然后重试。文件正在使用时，Windows 无法更新应用。如果仍然失败，请重启 Windows，并在打开应用前重试。 如果没有打开的应用窗口，请通过任务管理器关闭应用。
prepare-install-busy = 其他安装或待完成的重启阻止了更新。请等待安装完成，按提示重启 Windows，然后重试。
prepare-error-code = 错误代码：{ $code }
prepare-open-store = 打开 Microsoft Store

check-user-account = 用户账户
detail-user-account-ok = UAC 已启用，您的账户已准备好进行安装。
