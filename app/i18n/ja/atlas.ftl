...
prepare-failed = 一部の更新を完了できませんでした。以下のエラーを確認し、問題を解消してから再試行してください。
...
prepare-progress-unchanged = { $minutes } 分間、進捗の変化が報告されていません。大きな更新には時間がかかる場合があります。詳細は診断を開いて確認してください。
prepare-report-delayed = { $seconds } 秒間、状態を受信していません。診断を開いて更新プロセスを確認してください。

prepare-affected-app = 対象のアプリ
prepare-app-in-use = { $app } とそのウィンドウを閉じて、再試行してください。ファイルが使用中のため更新できません。失敗が続く場合は Windows を再起動し、アプリを開く前に再試行してください。 ウィンドウが開いていない場合は、タスク マネージャーでアプリを終了してください。
