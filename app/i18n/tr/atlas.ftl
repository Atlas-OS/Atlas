...
prepare-failed = Bazı güncellemeler tamamlanamadı. Aşağıdaki hatayı inceleyin, sorunu giderin ve tekrar deneyin.
...
prepare-progress-unchanged = { $minutes } dk boyunca ilerleme bildirilmedi. Büyük güncellemeler zaman alabilir. Ayrıntılar için tanılamayı açın.
prepare-report-delayed = { $seconds } sn boyunca durum bilgisi alınmadı. Güncelleme işlemini kontrol etmek için tanılamayı açın.

prepare-affected-app = etkilenen uygulamayı
prepare-app-in-use = { $app } ve pencerelerini kapatıp tekrar deneyin. Dosyaları kullanımdayken Windows uygulamayı güncelleyemez. Sorun sürerse Windows’u yeniden başlatın ve uygulamayı açmadan tekrar deneyin. Açık uygulama penceresi yoksa uygulamayı Görev Yöneticisi üzerinden kapatın.
