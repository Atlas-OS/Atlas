...
foreach ($manifest in @($coreManifest, $shellManifest)) {
    if (-not [IO.File]::Exists($manifest)) {
        throw "The protected module manifest is missing at '$manifest'."
    }
...
try {
    ...
    $failure = $_
    Write-AtlasNotApplied -Title 'Take Ownership' -Reason $_.Exception.Message
}

if ($Pause) {
    if ($null -eq $failure) {
        Write-AtlasCompletion -Title "Take Ownership of $canonicalPath"
    }
    Wait-AtlasExit
}
if ($null -ne $failure) {
