...
$trustBootstrap = [IO.Path]::GetFullPath([IO.Path]::Combine($PSScriptRoot, '..\Initialize-AtlasPowerShell.ps1'))
...
# The console belongs to the launcher that started this script; it prints through the
# shared vocabulary and leaves the exit pause to that launcher.
if (-not $Silent) {
...
function Enable-MicrosoftUpdate {
    ...
    if (-not $Silent) {
        Write-AtlasStep -Text 'Registering Microsoft Update as the driver source...'
    }
...
function Update-Drivers {
    ...
    if (-not $Silent) {
        Write-AtlasStep -Text 'Checking Microsoft Update for driver updates. This can take a few minutes...'
    }
    try {
        ...
        Write-AtlasLog -Level Error -Message "Failed to query driver updates: $($_.Exception.Message)" -ErrorRecord $_ -NoConsole
        Write-AtlasFailure -Text "Driver updates could not be checked: $($_.Exception.Message)"
        return $false
    }
    ...
    if ($updates.Count -eq 0) {
        ...
        if (-not $Silent) {
            Write-AtlasNote -Text 'No driver updates are available for this PC.'
        }
        return $true
    }
    ...
    if ($Silent) {
        ...
        Write-AtlasNote -Text "$($updates.Count) driver update(s) are available. Choose the ones to install in the window that opened."
    ...
    if ($selection.Count -eq 0) {
        ...
        if (-not $Silent) {
            Write-AtlasNote -Text 'No drivers were selected, so nothing was installed.'
        }
        return $true
    }
    ...
    if (-not $Silent) {
        Write-AtlasStep -Text "Downloading and installing $($selection.Count) driver update(s). This can take a while..."
        Write-AtlasNote -Text ([string[]]@($selection | ForEach-Object { "  - $($_.Title.ToString().Trim())" }))
    }
    ...
    try {
        ...
        Write-AtlasLog -Level Error -Message "Driver update installation failed: $($_.Exception.Message)" -ErrorRecord $_ -NoConsole
        Write-AtlasFailure -Text "The driver updates were not installed: $($_.Exception.Message)"
        return $false
    }

    Write-AtlasLog -Message "Driver updates installed successfully!"
    if (-not $Silent) {
        Write-AtlasSuccess -Text "Installed $($installation.InstalledCount) driver update(s)."
    }
    ...
    if (-not $Silent) {
        if (Read-AtlasYesNo -Question 'Restart Windows now?') {
            ...
            Write-AtlasStep -Text 'Restarting Windows in 10 seconds...'
            Start-Sleep -Seconds 10
            Microsoft.PowerShell.Management\Restart-Computer -Force -ErrorAction Stop
        }
    }
...
try {
    ...
    Write-AtlasLog -Level Error -Message "Driver update process failed: $($_.Exception.Message)" -ErrorRecord $_ -NoConsole
    Write-AtlasFailure -Text "The driver update did not complete: $($_.Exception.Message)"
    exit 1
}
