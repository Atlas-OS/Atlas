    ...
    Categories = @(
        ...
        @{
            ...
            Tweaks = @(
                ...
                # config-start-menu is still YAML (invokes the top-level STARTMENU.ps1)
                ...
                # taskbar/config-pins is still YAML (invokes the top-level TASKBARPINS.ps1)
        ...
        @{
            ...
            Tweaks = @(
                ...
                # create-shortcuts is still YAML (invokes the top-level SHORTCUTS.ps1)
                'add-newUser-script'
                # add-music-videos-to-home is still YAML (needs unelevated user-context Shell COM)
                # enable-notifications is still YAML (invokes the top-level ENABLENOTIFS.cmd)
