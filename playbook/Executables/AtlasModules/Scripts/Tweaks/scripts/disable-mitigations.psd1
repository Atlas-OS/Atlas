@{
    ...
    Run         = @(
        @{
            ...
            Args = @('-NoLogo', '-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-File', '{windir}\AtlasModules\Scripts\Entry\Invoke-Toggle.ps1', '-Name', 'Mitigations', '-State', 'Disable', '/silent')
            Wait = $true
        }
    )
}
